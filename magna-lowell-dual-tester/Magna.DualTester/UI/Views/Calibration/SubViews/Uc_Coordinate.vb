...
Public Class Uc_Coordinate
    ...
    Private Sub UpdateRawvaluesRefernce()
        ...
        If _GlassType = GlassTypeEnum.LH_CVX OrElse _GlassType = GlassTypeEnum.LH_FLAT Then
            V_Sample_RawMax.Enabled = VertRawUp > 250 AndAlso VertRawUp <> 0
            V_Sample_RawMin.Enabled = VertRawDown < 100 AndAlso VertRawDown <> 0
        ...
        H_Sample_RawMin.Enabled = HorzRawIb < 400 AndAlso HorzRawIb <> 0
        H_Sample_RawMax.Enabled = HorzRawOb > 450 AndAlso HorzRawOb <> 0
    ...
    Private Function ValidateData() As Udt_Mcal
        Dim Mc = GetValues()
        With Mc
            If Math.Abs(.H.EuMax - .H.EuMin) > 8 AndAlso Math.Abs(.V.EuMax - .V.EuMin) > 8 Then
                If (.H.RawMax - .H.RawMin) > 100 Then
